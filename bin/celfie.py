...
def em(x, x_depths, y, y_depths, num_iterations, convergence_criteria, seed):
...
def parse_header_samples(header):
...
if __name__ == "__main__":
    ...
    output_alpha_file = f"{args.parallel_job_id}_tissue_proportions.txt"
    output_gamma_file = f"{args.parallel_job_id}_methylation_proportions.txt"
    ...
    # Generate a list of random seeds that are always the same to ensure reproducibility
    initial_seed = 42
    np.random.seed(initial_seed)
    random_seeds = np.random.randint(low=0, high=10000, size=args.random_restarts)

    for i in range(args.random_restarts):
        alpha, gamma, ll = em(
            x, x_depths, y, y_depths, args.max_iterations, args.convergence, random_seeds[i]
        )
