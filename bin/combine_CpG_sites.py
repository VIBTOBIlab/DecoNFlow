...
def usage():
  sys.stderr.write('''Usage: python combine_CpG_sites.py  [options]  -i <input>+ -o <output>
    -i <input>+    One or more files, each listing methylation counts
...
def main():
  ...
  while i < len(args):
      if args[i] == '-h' or args[i] == '--help':
            ...
            i += 1  # Move to the next argument after '-i'
            ...
            sys.stderr.write('Error! Unknown parameter with no arg: ' + '%s\n' % args[i])
            usage()
      i += 1
